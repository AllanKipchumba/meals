        ...
        if 'dinner_recipes' in st.session_state or 'breakfast_recipes' in st.session_state:
            ...
            try:                
                ...
                for i, recipe in enumerate(st.session_state['dinner_recipes'], 1):                
                    ...
                    db.create_recipe(
                        user_id=st.session_state['user'].get("id"),  # Assuming user_id is stored in session state
                        name=recipe['name'],
                        ingredients=recipe['ingredients'],
                        instructions=recipe['instructions'],
                        calories=int(recipe['calories']),  # Convert to integer
                        protein=float(recipe['macros']['protein'].split()[0]),  # Extract numeric value
                        carbs=float(recipe['macros']['carbs'].split()[0]),  # Extract numeric value
                        fat=float(recipe['macros']['total_fat'].split()[0]),  # Extract numeric value
                        tags=recipe.get('tags', []),  # Default to empty list if tags are missing
                        difficulty=recipe['difficulty'],
                        dietary_info=recipe['dietary_info'],
                        cooking_time=int(recipe['cooking_time'].split()[0]),  # Extract numeric value
                        prep_time=20,  # Add if available in the recipe
                        weekend_prep=recipe['weekend_prep'],
                        servings=30,  # Add if available in the recipe                        
                    )
                ...
                for i, recipe in enumerate(st.session_state['breakfast_recipes'], 1):
                    db.create_recipe(
                        ...
                        dietary_info=recipe['dietary_info'],
                        cooking_time=int(recipe['cooking_time'].split()[0]),  # Extract numeric value
                        prep_time=30,  # Add if available in the recipe
                        weekend_prep=recipe['weekend_prep'],
                        servings=30,  # Add if available in the recipe
